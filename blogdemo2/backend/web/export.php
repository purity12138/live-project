...
$win=explode(",", $_POST['win']);
$nam=explode(",", $_POST['name']);
/**
 * Created by lonm.shi.
 * Date: 2012-02-09
 * Time: 下午4:54
...
for($i=0;$i<$_POST['count'];$i++){
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$j, $name[$i]);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$j, $serial[$i]);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$j, $win[$i+1]);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$j, $nam[$i+1]);
    $j++;
}
